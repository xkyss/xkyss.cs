...
**启动项(Launcher Item)**:
应用启动管理器管理的单个可启动条目(程序、脚本、URL 等),包含名称与启动命令,并归属分类树中某个分类节点;是 Launcher 的数据核心。
_Avoid_: item、条目(文档中用「启动项」)

**分类(Category)**:
Launcher 中组织启动项的多级树形分组:树中每个节点有唯一 slug id 与显示名称,可含子分类;启动项通过 id 归属某个分类节点,移动/重命名分类不影响启动项数据。
_Avoid_: 分组、标签、文件夹(文档中用「分类」)

**未分类(Uncategorized)**:
分类树的固定节点(树底收尾,不可管理):聚合所有未归属任何分类节点的启动项,「全部」视图包含它;旧模型的「默认」分类迁移后归入此处。
